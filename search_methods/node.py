...
class TreeNode:
    def __init__(self, state: BoardState, board: Board, search_method: SearchMethod, cost: int, is_goal: bool) -> None:
        self.state = state
        self.board = board
        self.cost = cost
    ...
    def expand(self) -> List['TreeNode']:
        for direction in self.directions:
            ...
            new_node = TreeNode(new_state, self.board, self.search_method, self.cost + 1, move_result)
    ...
    def get_heuristic(self) -> int:
        return self.search_method.calculate_heuristic(self.state)
